...
import {ColorTheme, MessageOptions, showMessage} from "react-native-flash-message";
...
const CreateAnAccount = ({navigation} : {navigation : any}) => {
	...
	const sendOtp = async () => {
		try {
			...
			navigation.navigate(Routes.RegistrationOTPVerificationScreen, {defaultEmailValue: defaultEmailValue});
			showDescFlashMessage('Success', "OTP has been sent successfully to: " + defaultEmailValue , 'success');

		} catch (error) {
			console.error("CreateAnAccount: sendOtp(): " + error);
			showDescFlashMessage('Error sending OTP', "Enter a valid email", 'danger');
		}
	};
	const showFlashMessage = (message: string, type: 'danger' | 'warning' | 'success') => {
		showMessage({
			message: message,
			type: type,
			autoHide: true,
	...
	const showDescFlashMessage = (message: string, description: string, type: 'danger' | 'warning' | 'success') => {
		showMessage({
			message: message,
			type: type,
			autoHide: true,
